...
def action_prompt_text() -> str:
    return (
        '🧭 <b>Выбор действия</b>\n<blockquote>'
        'Выбери формат: одно сообщение или чат. '
...
def waiting_message_text(is_reply: bool) -> str:
    if is_reply:
        return system_text(
            'Жду ответ',
            'Пришли ответ. Я передам его анонимно и сохраню форматирование.',
            '↩️',
        )
    return system_text(
        'Жду сообщение',
        'Пришли сообщение. Поддерживаются текст, фото, видео, стикеры, GIF, голосовые и другое.',
        '🟢',
    )
